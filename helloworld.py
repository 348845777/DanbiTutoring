...
print()
gameInstance = GameInstance(window, my_font, countdownTimer, gameComplete, clickCounter)
objects = []
...
def checkEvent(event, window, instance):
    global run, randx, randy, text_surface, clickobject
    if not instance.gameComplete:
        if event.type == pygame.MOUSEBUTTONDOWN:
            click = rect.collidepoint(pygame.mouse.get_pos())
            
            if click == 1:
                instance.clickCounter = instance.clickCounter +1
                randx = random.randint(0, monitors[0].width - squareDimension)

                randy = random.randint(0, monitors[0].height - squareDimension)
            clickobject = my_font.render(str(instance.clickCounter), False, (255,255,0))    

        elif event.type == pygame.USEREVENT: 
            if instance.countdownTimer == 0: 
                instance.gameComplete = True
            else:
                instance.countdownTimer = instance.countdownTimer -1
            text_surface = my_font.render(str(instance.countdownTimer), False, (255,255,0))
...
def myFunction(instance):
    ...
    instance.clickCounter = 0
    instance.countdownTimer = 10

customButton = Button(30, 30, 400, 100, 'Button One (onePress)', myFunction(gameInstance))
customButton1 = Button(30, 140, 400, 100, 'Button Two (multiPress)', myFunction(gameInstance), True)

while run is True:
    window.fill((0, 0, 0))
    for event in pygame.event.get():
        checkEvent(event, window, gameInstance)
